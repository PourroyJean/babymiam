...
function buildSignaturePayload(userId, issuedAtEpochSeconds) {
  return `${SIGNATURE_NAMESPACE}:${userId}:${issuedAtEpochSeconds}`;
}
...
function computeSharedTestLoginSignature({ userId, issuedAtEpochSeconds, secret }) {
  if (!isPositiveInteger(userId)) {
    throw new Error("Invalid shared test login token user id.");
  }

  if (!isPositiveInteger(issuedAtEpochSeconds)) {
    throw new Error("Invalid shared test login token issued-at epoch seconds.");
  }
  ...
  return crypto
    ...
    .update(buildSignaturePayload(userId, issuedAtEpochSeconds))
    .digest("hex");
}
...
function parseSharedTestLoginToken(token) {
  ...
  return { userId, issuedAtEpochSeconds, signature };
}

function createSharedTestLoginToken({ userId, issuedAtEpochSeconds, secret }) {
  const signature = computeSharedTestLoginSignature({ userId, issuedAtEpochSeconds, secret });
  return `${userId}.${issuedAtEpochSeconds}.${signature}`;
}
