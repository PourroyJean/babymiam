...
CREATE INDEX IF NOT EXISTS idx_food_tastings_owner_tasted_on
  ON food_tastings(owner_id, tasted_on DESC, updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_food_tastings_owner_updated_at
  ON food_tastings(owner_id, updated_at DESC);

INSERT INTO food_tastings (owner_id, food_id, slot, tasted_on)
SELECT
  p.owner_id,
  p.food_id,
  generated.slot,
  COALESCE(p.first_tasted_on, CURRENT_DATE)
FROM food_progress p
CROSS JOIN LATERAL generate_series(1, LEAST(GREATEST(p.exposure_count, 0), 3)) AS generated(slot)
ON CONFLICT (owner_id, food_id, slot)
DO NOTHING;

WITH tasting_stats AS (
  SELECT
    owner_id,
    food_id,
    COUNT(*)::int AS exposure_count,
    MIN(tasted_on)::date AS first_tasted_on
  FROM food_tastings
  GROUP BY owner_id, food_id
)
UPDATE food_progress p
SET
  exposure_count = tasting_stats.exposure_count,
  first_tasted_on = tasting_stats.first_tasted_on,
  updated_at = NOW()
FROM tasting_stats
WHERE p.owner_id = tasting_stats.owner_id
  AND p.food_id = tasting_stats.food_id
  AND (
    p.exposure_count IS DISTINCT FROM tasting_stats.exposure_count
    OR p.first_tasted_on IS DISTINCT FROM tasting_stats.first_tasted_on
  );
...
WHERE NOT EXISTS (
  ...
  WHERE t.owner_id = p.owner_id
    AND t.food_id = p.food_id
)
AND (
  p.exposure_count <> 0
  OR p.first_tasted_on IS NOT NULL
);
